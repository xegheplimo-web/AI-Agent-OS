...
- **CI status**: CI đã chạy xanh trên GitHub (PR #4, run #30576409860). Tất cả 4 job pass: verify (2m38s) + tauri-build Windows nsis (7m15s) + macOS dmg (4m50s) + Linux deb (4m45s). Installer artifacts được upload thật. `npm audit` remain `continue-on-error` vì 16 vulnerabilities đều từ `sharp` (inherited next.js, không fix không breaking).
